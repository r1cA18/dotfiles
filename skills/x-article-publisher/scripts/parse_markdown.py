...
def find_image_file(original_path: str, filename: str) -> tuple[str, bool]:
    ...
    print(
        f"[parse_markdown] WARNING: Image not found: '{original_path}' (also searched {[str(d) for d in SEARCH_DIRS]})",
        file=sys.stderr,
    )
...
def parse_markdown_file(filepath: str) -> dict:
        ...
        if end_marker != -1:
            ...
            for line in frontmatter.split("\n"):
                ...
                if line.startswith("title:"):
                    frontmatter_title = line[6:].strip().strip('"').strip("'")
                    break
    ...
    # Prefer frontmatter title over extracted title
    ...
    if missing:
        print(
            f"[parse_markdown] WARNING: {len(missing)} image(s) not found",
            file=sys.stderr,
        )
...
def main():
    ...
    parser.add_argument(
        "--html-only", action="store_true", help="Output only HTML content"
    )
