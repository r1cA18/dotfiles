...
ブランチ `codex/nix-modernize` の作業メモ。ryoppippi の dotfiles を参考に、
自分の Nix 構成を近代化するための調査結果と今後の方針を記録する。

> 出典: Claude Code セッション `260be7c3`（2026-06-02）の比較調査。
> このドキュメントは「調査だけ完了・実装ゼロ」の状態を引き継ぐためのもの。
> 次にこのブランチ（または派生ブランチ）で作業を始めるとき、まずこれを読む。
...
| treefmt / git-hooks | flake-module 組込（pre-commit で treefmt + deadnix + statix） | 無し |
...
### このブランチ（nix-modernize）でやる — nix 構成の本丸
...
3. **treefmt + git-hooks** — treefmt(nixfmt) + deadnix + statix を pre-commit に組込
4. **`nix run .#switch`** — 外部 `nh` 依存を減らし flake app として switch/build を定義（上記のついで）

### 別ブランチでやる — claude 運用系（nix とテーマが別）
...
## 次にこのブランチで作業を始めるときの手順

```bash
git checkout codex/nix-modernize   # main 最新 c70e6fd から切り直し済み
