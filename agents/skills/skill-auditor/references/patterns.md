...
- 複数の独立した機能があり分離すると選択しやすい
- 各機能が独自の指示を必要とする
- ユーザーが「〜の機能だけ使いたい」と言える粒度がある

### 構成例（補助directoryは必要なものだけ）
...
```
スキルの機能数は？
├── 独立したworkflowを選択する → Type 1: Router + Sub-skills
├── 1つだが、ドメイン知識が必要
│   ├── 独立したルールを選択して読む → Type 3: Reference Library
│   └── 少数の参照資料で十分 → Type 2: Single + References
└── 1つで自己完結する → Type 4: Simple
```
